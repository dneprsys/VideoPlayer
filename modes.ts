...
visible in the scene. For each object, provide the name (with representative emoji).`,
  },

  'Object Detection': {
    emoji: '📦',
    prompt: `Analyze the video to detect objects. Call set_timecodes_with_objects \
with the timecode, a description of the scene, and a list of objects \
visible in the scene. For each object, provide the name (with representative emoji), the confidence score (0-1), and the bounding box coordinates (ymin, xmin, ymax, xmax) normalized to 0-1.`,
  },

  Subtitles: {
    emoji: '💬',
    prompt: `Generate subtitles for this video. For each line of dialogue, \
call set_timecodes with the timecode and the spoken text.`,
    isList: true,
  },
